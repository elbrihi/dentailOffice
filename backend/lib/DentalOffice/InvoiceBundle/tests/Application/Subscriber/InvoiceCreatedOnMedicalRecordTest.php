...
class InvoiceCreatedOnMedicalRecordTest extends InvoiceTest
{
    ...
    private InvoiceCreatedOnMedicalRecord $invoiceOnMedicalRecord;
    ...
    public function test_intiale_invoice()
      {
        ...
        $invoice = $this->entityManager
            ->getRepository(InvoiceOrmEntity::class)
        ...
        $invoiceItems = $this->entityManager
            ...
            ->findBy(['invoiceOrmEntity' => $invoice]);

        $this->assertCount(3, $invoiceItems, 'Should create 3 invoice items');

        // 3. Total calculation
        $expectedTotal = array_sum(array_column($items, 'amount'));

        $this->assertEquals(
            $expectedTotal,
            $invoice->getTotalAmount(),
            'Total amount should equal sum of items'
        );

        // 4. Remaining due
        $expectedRemaining = $expectedTotal - $invoice->getTotalPaid();

        $this->assertEquals(
            $expectedRemaining,
            $invoice->getRemainingDue(),
            'Remaining due should be correct'
        );

        // 5. Check each item integrity
